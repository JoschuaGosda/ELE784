...
//attachez a nos urb
//call quand urb se termine
//il reste a se conforme pour l<interaction avec pilote et usager
//mettre qqpart pour que lusager peuvent lobtenir avec un read
static void complete_callback(struct urb *urb) {
	...
	if (urb->status == 0) {
		for (i = 0; i < urb->number_of_packets; ++i) {
			...
			if (LastFID != (UrbPacketData[1] & STREAM_FID)) {	// Détection du début d'un nouveau Frame
				buffer->Status &= ~BUF_STREAM_EOF;
				LastFID = (UrbPacketData[1] & STREAM_FID);
